...
# Scatter plots for attribute pairs
for i in range(len(attributes)-1):
    for j in range(i+1, len(attributes)-1):
        plt.figure(figsize = (5, 5))
        for label in df['Disease'].unique():
            df_filtered = df[df['Disease'] == label]
            plt.scatter(df_filtered[attributes[i]], df_filtered[attributes[j]], alpha = 0.5, label = label)
        plt.title(f'{attributes[i]} vs {attributes[j]}')
